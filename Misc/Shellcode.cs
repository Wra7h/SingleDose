using System;
using System.Collections.Generic;
using System.IO;
using System.Runtime.InteropServices;
using System.Text;

namespace SingleDose.Misc
{
    public class ShellcodeHistory
    {
        public string Path { get; set; }
        public byte[] Shellcode { get; set; }
    }

    class Shellcode
    {
        public static List<ShellcodeHistory> History = new List<ShellcodeHistory>();

        // Displays each entry in the history 
        public static void DisplayHistory()
        {
            if (History.Count == 0)
            {
                SDConsole.WriteError("No entries found.");
                return;
            }

            SDConsole.Write("");
            //SDConsole.WriteInfo("INJECT HISTORY");
            for (int i = 0; i < History.Count; i++)
            {
                Console.ForegroundColor = ConsoleColor.Cyan;
                Console.Write("       H{0}: ", i + 1);
                Console.ResetColor();
                Console.WriteLine("{0} [Size: {1}]", History[i].Path, History[i].Shellcode.Length);
                SDConsole.iConsoleLineNum++;
            }
            SDConsole.Write("");
        }
    }

    internal class SRDI
    {
        public static byte[] Generate(string path)
        {
            byte[] dllContents = File.ReadAllBytes(path); //DLL the user specified
            string functionNameResponse;
            uint functionHash = 0;

            SDConsole.Write("[OPTIONAL] Enter the name of an exported function to call after DLLMain:");
            Console.Write("       > ");
            functionNameResponse = Console.ReadLine();
            if ((functionNameResponse != null) && (functionNameResponse != ""))
            {
                functionHash = HashFunction(functionNameResponse); //Function name to call after DLLMain
            }

            string userDataResponse;
            byte[] userData = new byte[] { };
            SDConsole.Write("[OPTIONAL] Enter any additional data to pass to call after DLLMain:");
            Console.Write("       > ");
            SDConsole.iConsoleLineNum++;
            userDataResponse = Console.ReadLine();
            if ((userDataResponse != null) && (userDataResponse != ""))
            {
                userData = Encoding.Default.GetBytes(userDataResponse);
            }

            string obfuscateImportsResponse;
            bool obfuscateImports = false;
            SDConsole.Write("[OPTIONAL] Would you like to Obfuscate imports? (Y/N)");
            do
            {
                Console.Write("       > ");
                SDConsole.iConsoleLineNum++;
                obfuscateImportsResponse = Console.ReadLine();
            } while (obfuscateImportsResponse.ToUpper() != "Y" && obfuscateImportsResponse.ToUpper() != "N" && obfuscateImportsResponse != "" && obfuscateImportsResponse.ToLower() != "exit");

            if (obfuscateImportsResponse.ToLower() == "exit")
            {
                return null;
            }

            string importDelayResponse;
            int importDelay;
            if (obfuscateImportsResponse.ToUpper() == "Y")
            {
                obfuscateImports = true;
                SDConsole.Write("[OPTIONAL] Enter seconds to pause between loading imports? (Default: 0)");
                //SDConsole.iConsoleLineNum++;
                do
                {
                    Console.Write("   > ");
                    SDConsole.iConsoleLineNum++;
                    importDelayResponse = Console.ReadLine();
                } while (!int.TryParse(importDelayResponse, out importDelay) && importDelayResponse != "" && importDelayResponse.ToLower() != "exit");

                if (importDelayResponse.ToLower() == "exit")
                {
                    return null;
                }
            }
            else
            {
                importDelay = 0;
            }

            string clearHeaderResponse;
            bool clearHeader = false;
            SDConsole.Write("[OPTIONAL] Would you like to clear the PE header on load? (Y/N)");
            do
            {
                Console.Write("       > ");
                SDConsole.iConsoleLineNum++;
                clearHeaderResponse = Console.ReadLine();
            } while (clearHeaderResponse.ToUpper() != "Y" && clearHeaderResponse.ToUpper() != "N" && clearHeaderResponse != "" && clearHeaderResponse.ToLower() != "exit");

            if (clearHeaderResponse.ToLower() == "exit")
            {
                return null;
            }

            if (clearHeaderResponse.ToUpper() == "Y")
            {
                clearHeader = true;
            }
            else if (clearHeaderResponse.ToUpper() == "N")
            {
                clearHeader = false;
            }

            uint flags = 0;
            if (clearHeader)
            {
                flags = flags | 0x1;
            }

            if (obfuscateImports)
            {
                double delay = importDelay * Math.Pow(2, 16);
                flags = ((flags | 0x4) | (uint)delay);
            }
            SDConsole.iConsoleLineNum++;
            return ConvertToSC(dllContents, functionHash, userData, flags);
        }
        private static uint Ror(uint val, int r_bits, int max_bits)
        {
            return (val >> r_bits) | (val << (max_bits - r_bits));
        }
        private static uint HashFunction(string name)
        {
            uint functionHash = 0;
            name += "\x00";
            foreach (char c in name)
            {
                functionHash = Ror(functionHash, 13, 32);
                functionHash += c;
            }
            return functionHash;
        }
        private static bool Is64BitDLL(byte[] dllBytes)
        {
            bool is64Bit = false;
            GCHandle scHandle = GCHandle.Alloc(dllBytes, GCHandleType.Pinned);
            IntPtr scPointer = scHandle.AddrOfPinnedObject();
            Int32 headerOffset = Marshal.ReadInt32(scPointer, 60);
            UInt16 magic = (UInt16)Marshal.ReadInt16(scPointer, headerOffset + 4);
            if (magic == (UInt16)512 || magic == (UInt16)34404)
                is64Bit = true;
            scHandle.Free();
            return is64Bit;
        }
        private static byte[] Combine(byte[] first, byte[] second)
        {
            byte[] bytes = new byte[first.Length + second.Length];
            Buffer.BlockCopy(first, 0, bytes, 0, first.Length);
            Buffer.BlockCopy(second, 0, bytes, first.Length, second.Length);
            return bytes;
        }
        private static byte[] ConvertToSC(byte[] dllBytes, uint functionHash, byte[] userData, uint flags)
        {

            byte[] rdiShellcode32 = new byte[] {  };
            byte[] rdiShellcode64 = new byte[] {  };

            byte[] newShellcode = new byte[] { };
            uint dllOffset = 0;

            if (Is64BitDLL(dllBytes))
            {
                byte[] rdiShellcode = rdiShellcode64;
                int bootstrapSize = 64;

                // call next instruction (Pushes next instruction address to stack)
                newShellcode = Combine(newShellcode, new byte[] { 0xe8, 0x00, 0x00, 0x00, 0x00 });
                // Set the offset to our DLL from pop result
                dllOffset = (uint)(bootstrapSize - newShellcode.Length + rdiShellcode.Length);
                // pop rcx - Capture our current location in memory
                newShellcode = Combine(newShellcode, new byte[] { 0x59 });
                // mov r8, rcx - copy our location in memory to r8 before we start modifying RCX
                newShellcode = Combine(newShellcode, new byte[] { 0x49, 0x89, 0xc8 });
                // Setup the location of the DLL into RCX
                // add rcx, <Offset of the DLL>
                newShellcode = Combine(newShellcode, new byte[] { 0x48, 0x81, 0xc1 });
                foreach (byte b in BitConverter.GetBytes(dllOffset))
                {
                    //newShellcode.Add(b);
                    newShellcode = Combine(newShellcode, new byte[] { b });
                }
                // mov edx, <Hash of function>
                newShellcode = Combine(newShellcode, new byte[] { 0xba });
                foreach (byte b in BitConverter.GetBytes(functionHash))
                {
                    //newShellcode.Add(b);
                    newShellcode = Combine(newShellcode, new byte[] { b });
                }
                // Put the location of our user data in 
                // add r8, <Offset of the DLL> + <Length of DLL>
                newShellcode = Combine(newShellcode, new byte[] { 0x49, 0x81, 0xc0 });
                foreach (byte b in BitConverter.GetBytes((uint)(dllOffset + dllBytes.Length)))
                {
                    //newShellcode.Add(b);
                    newShellcode = Combine(newShellcode, new byte[] { b });
                }
                // mov r9d, <Length of User Data>
                newShellcode = Combine(newShellcode, new byte[] { 0x41, 0xb9 });
                foreach (byte b in BitConverter.GetBytes((uint)userData.Length))
                {
                    newShellcode = Combine(newShellcode, new byte[] { b });
                }
                // push rsi - save original value
                newShellcode = Combine(newShellcode, new byte[] { 0x56 });
                // mov rsi, rsp - store our current stack pointer for later
                newShellcode = Combine(newShellcode, new byte[] { 0x48, 0x89, 0xe6 });
                // and rsp, 0x0FFFFFFFFFFFFFFF0 - Align the stack to 16 bytes
                newShellcode = Combine(newShellcode, new byte[] { 0x48, 0x83, 0xe4, 0xf0 });
                // sub rsp, 0x30 - Create some breathing room on the stack 
                // 32 bytes for shadow space + 8 bytes for last arg + 8 bytes for stack alignment
                // mov dword ptr [rsp + 0x20], <Flags> - Push arg 5 just above shadow space
                newShellcode = Combine(newShellcode, new byte[] { 0x48, 0x83, 0xec, 6 * 8, 0xc7, 0x44, 0x24, 4 * 8 });

                foreach (byte b in BitConverter.GetBytes((uint)flags))
                {
                    newShellcode = Combine(newShellcode, new byte[] { b });
                }
                // call - Transfer execution to the RDI
                newShellcode = Combine(newShellcode, new byte[] { 0xe8, (byte)(bootstrapSize - newShellcode.Length - 4), 0x00, 0x00, 0x00 });
                // mov rsp, rsi - Reset our original stack pointer
                newShellcode = Combine(newShellcode, new byte[] { 0x48, 0x89, 0xf4 });
                // pop rsi - Put things back where we left them
                newShellcode = Combine(newShellcode, new byte[] { 0x5e });
                // ret - return to caller
                newShellcode = Combine(newShellcode, new byte[] { 0xc3 });
                // Write the rest of RDI
                foreach (byte b in rdiShellcode)
                {
                    newShellcode = Combine(newShellcode, new byte[] { b });
                }
                // Write our DLL
                foreach (byte b in dllBytes)
                {
                    newShellcode = Combine(newShellcode, new byte[] { b });
                }
                // Write our userdata
                foreach (byte b in userData)
                {
                    newShellcode = Combine(newShellcode, new byte[] { b });
                }
            }
            else // 32 Bit
            {
                byte[] rdiShellcode = rdiShellcode32;
                int bootstrapSize = 49;
                // call next instruction (Pushes next instruction address to stack)
                newShellcode = Combine(newShellcode, new byte[] { 0xe8, 0x00, 0x00, 0x00, 0x00 });
                // Set the offset to our DLL from pop result
                dllOffset = (uint)(bootstrapSize - newShellcode.Length + rdiShellcode.Length);
                // pop eax - Capture our current location in memory
                newShellcode = Combine(newShellcode, new byte[] { 0x58 });
                // push ebp
                newShellcode = Combine(newShellcode, new byte[] { 0x55 });
                // mov ebp, esp
                newShellcode = Combine(newShellcode, new byte[] { 0x89, 0xe5 });
                // mov edx, eax - copy our location in memory to ebx before we start modifying eax
                newShellcode = Combine(newShellcode, new byte[] { 0x89, 0xc2 });
                // add eax, <Offset to the DLL>
                newShellcode = Combine(newShellcode, new byte[] { 0x05 });
                foreach (byte b in BitConverter.GetBytes(dllOffset))
                {
                    newShellcode = Combine(newShellcode, new byte[] { b });
                }
                // add edx, <Offset to the DLL> + <Size of DLL>
                newShellcode = Combine(newShellcode, new byte[] { 0x81, 0xc2 });
                foreach (byte b in BitConverter.GetBytes((uint)(dllOffset + dllBytes.Length)))
                {
                    newShellcode = Combine(newShellcode, new byte[] { b });
                }
                // push <Flags>
                newShellcode = Combine(newShellcode, new byte[] { 0x68 });
                foreach (byte b in BitConverter.GetBytes(flags))
                {
                    newShellcode = Combine(newShellcode, new byte[] { b });
                }
                // push <Length of User Data>
                newShellcode = Combine(newShellcode, new byte[] { 0x68 });
                foreach (byte b in BitConverter.GetBytes((uint)userData.Length))
                {
                    newShellcode = Combine(newShellcode, new byte[] { b });
                }
                // push edx
                newShellcode = Combine(newShellcode, new byte[] { 0x52 });
                // push <hash of function>
                newShellcode = Combine(newShellcode, new byte[] { 0x68 });
                foreach (byte b in BitConverter.GetBytes(functionHash))
                {
                    newShellcode = Combine(newShellcode, new byte[] { b });
                }
                // push eax
                newShellcode = Combine(newShellcode, new byte[] { 0x50 });
                // call - Transfer execution to the RDI
                newShellcode = Combine(newShellcode, new byte[] { 0xe8, (byte)(bootstrapSize - newShellcode.Length - 4), 0x00, 0x00, 0x00 });
                // add esp, 0x14
                newShellcode = Combine(newShellcode, new byte[] { 0x83, 0xc4, 0x14 });
                // leave
                newShellcode = Combine(newShellcode, new byte[] { 0xc9 });
                // ret - return to caller
                newShellcode = Combine(newShellcode, new byte[] { 0xc3 });
                //Write the rest of RDI
                foreach (byte b in rdiShellcode)
                {
                    newShellcode = Combine(newShellcode, new byte[] { b });
                }
                //Write our DLL
                dllBytes[0] = 0x00;
                dllBytes[1] = 0x00;
                foreach (byte b in dllBytes)
                {
                    newShellcode = Combine(newShellcode, new byte[] { b });
                }
                //Write our userdata
                foreach (byte b in userData)
                {
                    newShellcode = Combine(newShellcode, new byte[] { b });
                }
            }
            return newShellcode;
        }

        public class SRDIArgs //only used when building the "srdi" technique in static mode
        {
            public string HashFunction { get; set; }
            public string AdditionalData { get; set; }
            public bool ObfuscateImports { get; set; }
            public int ImportDelay { get; set; }
            public bool ClearHeader { get; set; }
            public string DLLFilepath { get; set; }
            public byte[] DLLData { get; set; }
        }
    }
}
