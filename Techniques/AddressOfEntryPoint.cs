...
namespace {{NAMESPACE}}
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            int returnLength;
            NtQueryInformationProcess(pi.hProcess, 0 /*ProcessBasicInformation*/, out pbi, Marshal.SizeOf(pbi), out returnLength);

            IntPtr pebOffset = (IntPtr)(pbi.PebBaseAddress.ToInt64() + 16);
            byte[] imageBase = new byte[IntPtr.Size];
            IntPtr numBytesRead;
            ReadProcessMemory(pi.hProcess, pebOffset, imageBase, imageBase.Length, out numBytesRead);

            byte[] headersBuffer = new byte[4096];
            IntPtr pImageBase = (IntPtr)(BitConverter.ToInt64(imageBase, 0));
            ReadProcessMemory(pi.hProcess, pImageBase, headersBuffer, 4096, out numBytesRead);
        ...
        {{ARGS}}
        #region PE_StructsAndEnumsForDays
